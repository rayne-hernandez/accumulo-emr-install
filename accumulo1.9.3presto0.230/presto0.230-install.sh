#!/bin/bash
#
# This script installs Presto 0.230 on an EMR instance with a connector to Accumulo 1.9.3 and authentication using an 
# LDAP server.
#
...
if [ ! -d "/home/hadoop/presto-server-0.230" ]
then
    cd /home/hadoop/
    ...
    # Untar tarball
    tar xzf presto-server-0.230.tar.gz
    # Create symlink for Presto distrubution 
    ln -s /home/hadoop/presto-server-0.230 /home/hadoop/presto-server
...
-XX:+ExitOnOutOfMemoryError'
    ...
    then 
        ...
        mv presto-cli-0.230-executable.jar bin/presto
        chmod +x bin/presto
        ...
        do
            ...
            then    
                ...
                do
                    ...
                    scp $PRESTO_HOME/plugin/accumulo/presto-accumulo-0.230.jar $CORE_IP_ADDRESS:$ACCUMULO_HOME/lib/ext
                done
                break
            fi
        done
