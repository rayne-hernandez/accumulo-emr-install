...
then
    ...
    #LDAP_HOST="ldap.kountable.com"
    LDAP_HOST="win-b4epe0r3e6s.ad2.dev.kountable.com"
    LDAP_AD_DOMAIN="ad2.dev.kountable.com"
    ENVIRONMENT="dev"
...
-XX:+ExitOnOutOfMemoryError'
    ...
    then 
        # Create Java Keystore file to store LDAP server TLS certificate 
        keytool -ext SAN=IP:$MASTER_IP_ADDRESS -genkeypair -alias presto -keyalg RSA -keystore etc/keystore.jks << EOF
changeit
changeit
$MASTER_IP_ADDRESS
...
EOF
        # Retrieve LDAP server TLS certificate 
        openssl s_client -showcerts -connect $LDAP_HOST:636 < /dev/null 2> /dev/null | openssl x509 -outform PEM > etc/ldap_server.crt
        # Import LDAP TLS certificate into Java keystore
        #keytool -import -keystore etc/keystore.jks -trustcacerts -alias ldap_server -file etc/ldap_server.crt << EOF
        sudo keytool -import -keystore $JAVA_HOME/lib/security/cacerts -trustcacerts -alias ldap_server -file etc/ldap_server.crt << EOF
changeit
yes
EOF
        ...
        echo "http-server.authentication.type=PASSWORD" >> etc/config.properties
        echo "http-server.https.enabled=true" >> etc/config.properties
        echo "http-server.https.port=8442" >> etc/config.properties
        echo "http-server.https.keystore.path=/home/hadoop/presto-server/etc/keystore.jks" >> etc/config.properties
        echo "http-server.https.keystore.key=changeit" >> etc/config.properties

        # Create authentication configuration file
        touch etc/password-authenticator.properties
        echo "password-authenticator.name=ldap" > etc/password-authenticator.properties
        echo "ldap.url=ldaps://$LDAP_HOST:636" >> etc/password-authenticator.properties
        #echo "ldap.user-bind-pattern=cn=\${USER},dc=kountable,dc=com" >> etc/password-authenticator.properties
        echo "ldap.user-bind-pattern=\${USER}@$LDAP_AD_DOMAIN" >> etc/password-authenticator.properties
        ...
        do
            ...
            then    
                ...
                do
                    ...
                    scp $PRESTO_HOME/plugin/accumulo/presto-accumulo-0.225.jar $CORE_IP_ADDRESS:$ACCUMULO_HOME/lib/
                done
                break
            fi
        done
